#!/usr/bin/env bash
# from: https://github.com/justjake/quickjs-emscripten/blob/main/examples/cloudflare-workers/copy-wasm-file-into-src.sh
#
...
for VARIANT in "${VARIANTS[@]}"; do
	...
	cp -v "$WASM_FILE" $VARIANT.wasm
	if [[ -f "$WASM_FILE.map" ]]; then
		cp -v "$WASM_FILE.map" $VARIANT.wasm.map.txt
	fi
done